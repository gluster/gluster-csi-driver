...
func ParseIntWithDefault(new string, defInt int) int {
	...
	if len(new) != 0 {
		...
		if err != nil {
			glog.Errorf("bad int string [%s], using default [%d]", new, defInt)
		} else {
			newInt = parsedInt
		}
	}
...
// ParseIntWithConstraints parses a string into an int, checking to see if the
// integer is between a minimum and a maximum value
func ParseIntWithConstraints(valueString string, min int, max int) (int, error) {
...
func ParseBoolWithDefault(new string, defBool bool) bool {
	...
	if len(new) != 0 {
		...
		if err != nil {
			glog.Errorf("bad bool string [%s], using default [%t]", new, defBool)
		} else {
			newBool = parsedBool
		}
	}
...
func ParseFloatWithDefault(new string, defFloat float32) float32 {
	...
	if len(new) != 0 {
		...
		if err != nil {
			glog.Errorf("bad float string [%s], using default [%f]", new, defFloat)
		} else {
