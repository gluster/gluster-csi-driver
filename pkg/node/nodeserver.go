...
func Run(config *command.Config) {
	srv := csi.NewNonBlockingGRPCServer()
	srv.Start(config.Endpoint, identity.NewServer(config), nil, NewServer(config))
	srv.Wait()
}
...
func (ns *Server) NodePublishVolume(ctx context.Context, req *api.NodePublishVolumeRequest) (*api.NodePublishVolumeResponse, error) {
	...
	if err = mountGlusterVolume(source, targetPath, mo); err != nil {
		return nil, err
	}
...
func mountGlusterVolume(source, targetPath string, mountOptions []string) error {
	err := glusterMounter.Mount(source, targetPath, "glusterfs", mountOptions)
	if err != nil {
