...
func TestProbe(t *testing.T) {
	...
	if result := assert.NoError(t, err); result == false {
		t.Fatal()
	}
	if result := assert.NotNil(t, resp); result == false {
		t.Fatal()
	}
	if result := assert.NotNil(t, resp.Ready); result == false {
		t.Fatal()
	}
	if result := assert.NotNil(t, resp.Ready.Value); result == false {
		t.Fatal()
	}
