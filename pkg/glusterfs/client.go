...
func (gc gd2Client) checkRespErr(orig error, kind, name string) error {
	...
	if errResp != nil {
		...
		} else if errResp.StatusCode == http.StatusConflict {
			return cli.ErrInProcess(kind, name)
		}
	}
	return orig
}
...
// GetControllerCapabilities returns a list of controller capabilities
func (gc gd2Client) GetControllerCapabilities() []csi.ControllerServiceCapability_RPC_Type {
...
func (gc gd2Client) CheckExistingVolume(volumeID string, volSizeBytes int64) error {
	...
	if volSizeBytes > 0 && int64(volInfo.Capacity) != volSizeBytes {
...
func (gc gd2Client) CreateVolume(volumeName string, volSizeBytes int64, params map[string]string) error {
	...
	for k, v := range params {
		...
		case "replicas":
			replicas := v
			replicaCount, err = cli.ParseIntWithConstraints(replicas, minReplicaCount, maxReplicaCount)
			if err != nil {
				return cli.ErrInvalidValue("replicas", err.Error())
			}

		default:
			return cli.ErrInvalidValue("parameters", fmt.Sprintf("invalid option %s given", k))
		}
	}
	...
	if err != nil {
		return gc.checkRespErr(err, "volume", volumeName)
	}
...
func (gc gd2Client) ListVolumes() ([]*csi.Volume, error) {
	...
	for _, vol := range vols {
		...
		volumes = append(volumes, &csi.Volume{
			Id:            vol.Name,
			CapacityBytes: int64(v.Size.Capacity),
		})
	}
...
func (gc gd2Client) CheckExistingSnapshot(snapName, volName string) (*csi.Snapshot, error) {
	...
	snap := gc.csiSnap(api.SnapInfo(snapInfo))
	return snap, nil
}
...
func (gc gd2Client) DeleteSnapshot(snapName string) error {
	...
	if err != nil && err.Error() != gd2Error.ErrSnapDeactivated.Error() {
...
func (gc gd2Client) ListSnapshots(snapName, srcVol string) ([]*csi.Snapshot, error) {
	...
	if len(snapName) != 0 {
		...
		if err != nil {
			return nil, gc.checkRespErr(err, "source volume", srcVol)
		}
